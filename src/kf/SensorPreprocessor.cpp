...
SensorPreprocessor::SensorPreprocessor(double imu_accel_noise, double imu_gyro_noise,
                                       double pos_noise)
    : imu_(imu_accel_noise, imu_gyro_noise),
      position_(pos_noise),
      integrated_vel_(Eigen::Vector3d::Zero())
{}

SensorPreprocessor::SensorPreprocessor(const boost::property_tree::ptree& pt)
    : imu_(pt.get<double>("sensor.imu_accel_noise", 0.01),
                                                                ...
                                                                int step, const StateVec& kf_est)
{
    ...
    measurement << pos_meas(0), pos_meas(1), pos_meas(2), integrated_vel_(0), integrated_vel_(1),
        integrated_vel_(2);
    return measurement;
}
...
const Eigen::Vector3d& SensorPreprocessor::getIntegratedVel() const { return integrated_vel_; }
